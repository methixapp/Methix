      ...
      <nav className="flex-grow">
        <ul>
          <li className="mb-6">
            <Link 
              href={homeItem.href} 
              className={`flex items-center p-2 rounded ${pathname === homeItem.href ? 'bg-gray-800' : 'hover:bg-gray-800'}`}
            >
          ...
          {navItems.map((item) => (
            <li key={item.href} className="mb-4">
              <Link 
                href={item.href} 
                className={`block p-2 rounded ${pathname === item.href ? 'bg-gray-800' : 'hover:bg-gray-800'}`}
              >
                {item.label}
              </Link>
            </li>
